...
class COVID19(object):
    ...
    def __init__(self, url="https://covid-tracker-us.herokuapp.com", data_source='jhu'):
        ...
        if url == self.externalSources.default_url:
            # Load mirrors
            response = requests.get(self.externalSources.mirrors_source)
            response.raise_for_status()
            self.externalSources.mirrors = response.json()

            # Try to get sources as a test
            for mirror in self.externalSources.mirrors:
                # Set URL of mirror
                self.externalSources.url = mirror["url"]
                result = None
                try:
                    result = self._getSources()
                except Exception as e:
                    # URL did not work, reset it and move on
                    self.externalSources.url = ""
                    continue
        ...
        self.externalSources.data_source = data_source
    ...
    def _getSources(self):
        response = requests.get(self.externalSources.url + "/v2/sources")
        response.raise_for_status()
    ...
    def _request(self, endpoint, params=None):
        if params is None:
            params = {}
        response = requests.get(self.externalSources.url + endpoint, {**params, "source":self.externalSources.data_source})
        response.raise_for_status()
